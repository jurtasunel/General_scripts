...
annotation = "/home/josemari/phoPR_analysis/Mbovis_reference_genome/LT708304_updated_25-05-18.gff"

#FEATURECOUNTS.
#Run featurecounts and store the output in a variable.
#ftCnts_table = featureCounts(files = c("R1_srtd.bam", "R2_srtd.bam", "R3_srtd.bam", "R4_srtd.bam", "R5_srtd.bam", "R6_srtd.bam", "R7_srtd.bam", "R8_srtd.bam", "R9_srtd.bam", "R10_srtd.bam", "R11_srtd.bam", "R12_srtd.bam"), isPairedEnd=TRUE, annot.ext = annotation, isGTFAnnotationFile = TRUE, GTF.featureType = "CDS", GTF.attrType = "db_xref", nthreads=3)
...
#write.table(ftCnts_table$counts, file = "ExpressionCounts.txt", sep = "\t",row.names = TRUE, col.names = TRUE)

#LOAD DATA.
#Read the table and store it in a variable.
ExpCounts <- read.table("ExpressionCounts.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE, row.names = 1)
...
res_Dpho_Control = results(dds, contrast = c("Design", "Dpho", "Control"))
res_Desx_Control = results(dds, contrast = c("Design", "Desx", "Control"))
res_Complesx_Control = results(dds, contrast = c("Design", "Complesx", "Control"))
res_Desx_Complesx = results(dds, contrast = c("Design", "Desx", "Complesx"))
#Visualization.
#head(res_Desx_Complesx, tidy = TRUE)
#summary(res_Desx_Complesx)

#Make volcano plots for each comparison.
volcano(res_Dpho_Control, "Dpho vs Control")
volcano(res_Desx_Control, "Desx vs Control")
volcano(res_Complesx_Control, "Complesx vs Control")
volcano(res_Desx_Complesx, "Desx vs Complesx")
...
DE_Dpho_Control = get_DE_genes(res_Dpho_Control, 3, 0.01)
DE_Desx_Control = get_DE_genes(res_Desx_Control, 3, 0.01)
DE_Complesx_Control = get_DE_genes(res_Complesx_Control, 3, 0.01)
DE_Desx_Complesx = get_DE_genes(res_Desx_Complesx, 3, 0.01)
...
#Important ot include last column as we created that variable with the names og the genes.
temp = vsDataFrame[rownames(DE_Complesx_Control), c(1:3, 7:9, 13)]
...
temp_long$GeneID <- factor(temp_long$GeneID, levels = rev(rownames(DE_Complesx_Control)))
#Plot the heatmap.
ggplot(temp_long, aes(x = variable, y = GeneID, fill = value)) + geom_tile() + xlab("Samples") + ylab("Gene ID") + ggtitle("COMPLESX DE GENES")+ theme(legend.title = element_text(size = 9), plot.title = element_text(size = 12), axis.title = element_text(size = 10, face = "bold"), axis.text.y = element_text(size = 8), axis.text.x = element_text(angle = 65, hjust = 1)) + labs(fill = "Reads")
